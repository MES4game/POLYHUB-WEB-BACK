...
export function choiceMapper<T, RK extends keyof T>(
    mapper: Mapper<T>,
    restrictedMapper: Mapper<Pick<T, RK>>,
    is_restricted: boolean,
): (obj: unknown) => T {
    if (!is_restricted) return mapper as (obj: unknown) => T;

    return (obj: unknown) => { return mapper(restrictedMapper(obj)); };
}
